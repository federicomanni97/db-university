...
SELECT `students`.`name`AS `nome` ,`students`.`surname` AS `cognome`, MAX(`exam_student`.`vote`) AS `voto_massimo`, COUNT(`exam_student`.`vote`) AS `numero_tentativi`, `courses`.`name`
FROM `students` 
JOIN `exam_student`
